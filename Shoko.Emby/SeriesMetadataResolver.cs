...
namespace Shoko.Emby
{
    public class SeriesMetadataResolver : IRemoteMetadataProvider<Series, SeriesInfo>
    {
        ...
        public async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(SeriesInfo info, CancellationToken cancellationToken)
        {
            ...
            var results = await Plugin.Instance.GetHttpAsync<List<Models.SeriesCollection>>($"group/search?query={info.Name}").ConfigureAwait(false);
        ...
        {
            var apiResult = await Plugin.Instance.GetHttpAsync<Models.Group.GroupResult>($"group?id={info.ProviderIds[Constants.ShokoGroup]}");
            ...
            apiResult.Roles.ForEach(role =>
            {
                ...
                {
                    ImageUrl = ((role.CharacterImage ?? role.StaffImage) != null) ? Plugin.Instance.Configuration.Url + (role.CharacterImage ?? role.StaffImage) : string.Empty,
                    Name = role.Staff,
                    Role = role.Character,
